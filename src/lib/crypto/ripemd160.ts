import { utf8Encode, rotl } from './utils';
...
export function ripemd160(str: string): string {
  str = utf8Encode(str);
  return binToHex(core_ripemd160(strToBin(str), str.length * 8));
}

function core_ripemd160(x: number[], len: number): number[] {
  // Append padding
  x[len >> 5] |= 0x80 << (len % 32);
  x[(((len + 64) >>> 9) << 4) + 14] = len;
  ...
  for (let i = 0; i < x.length; i += 16) {
    ...
    for (let j = 0; j < 80; j++) {
      ...
      // Left line
      t = (al + f(j, bl, cl, dl) + (x[i + r[j]] || 0) + K(j)) | 0;
      t = (rotl(t, s[j]) + el) | 0;
      al = el;
      el = dl;
      dl = rotl(cl, 10);
      ...
      // Right line
      t = (ar + f(79 - j, br, cr, dr) + (x[i + rr[j]] || 0) + Kr(j)) | 0;
      t = (rotl(t, sr[j]) + er) | 0;
      ar = er;
      er = dr;
      dr = rotl(cr, 10);
...
function binToHex(binArray: number[]): string {
