import { rotl, utf8Encode } from './utils';

/**
 * Calculates the SHA-1 hash of a string.
 */
export function sha1(str: string): string {
  str = utf8Encode(str);
  return binToHex(core_sha1(strToBin(str), str.length * 8));
}

function core_sha1(x: number[], len: number): number[] {
  /* append padding */
  x[len >> 5] |= 0x80 << (24 - len % 32);
  x[((len + 64 >> 9) << 4) + 15] = len;

  for (let i = 0; i < x.length; i++) {
    if (x[i] === undefined) x[i] = 0;
  }

  const w = new Array(80);
  let a = 1732584193;
  let b = -271733879;
  let c = -1732584194;
  let d = 271733878;
  let e = -1009589776;

  for (let i = 0; i < x.length; i += 16) {
    const olda = a;
    const oldb = b;
    const oldc = c;
    const oldd = d;
    const olde = e;

    for (let j = 0; j < 80; j++) {
      if (j < 16) w[j] = x[i + j];
      else w[j] = rotl(w[j - 3] ^ w[j - 8] ^ w[j - 14] ^ w[j - 16], 1);
      const t = (rotl(a, 5) + sha1_ft(j, b, c, d) + e + w[j] + sha1_kt(j)) | 0;
      e = d;
      d = c;
      c = rotl(b, 30);
      b = a;
      a = t;
    }

    a = (a + olda) | 0;
    b = (b + oldb) | 0;
    c = (c + oldc) | 0;
    d = (d + oldd) | 0;
    e = (e + olde) | 0;
  }
  return [a, b, c, d, e];
}

function sha1_ft(t: number, b: number, c: number, d: number): number {
  if (t < 20) return (b & c) | ((~b) & d);
  if (t < 40) return b ^ c ^ d;
  if (t < 60) return (b & c) | (b & d) | (c & d);
  return b ^ c ^ d;
}

function sha1_kt(t: number): number {
  return (t < 20) ? 1518500249 : (t < 40) ? 1859775393 :
         (t < 60) ? -1894007588 : -899497514;
}

/* Overwrite binToHex for SHA1 because it uses Big-Endian */
function binToHex(binArray: number[]): string {
  const hexTab = "0123456789abcdef";
  let str = "";
  for (let i = 0; i < binArray.length * 4; i++) {
    str +=
      hexTab.charAt((binArray[i >> 2] >> ((3 - i % 4) * 8 + 4)) & 0xf) +
      hexTab.charAt((binArray[i >> 2] >> ((3 - i % 4) * 8)) & 0xf);
  }
  return str;
}

/* Overwrite strToBin for SHA1 because it uses Big-Endian */
function strToBin(str: string): number[] {
  const bin: number[] = [];
  const mask = (1 << 8) - 1;
  for (let i = 0; i < str.length * 8; i += 8) {
    bin[i >> 5] |= (str.charCodeAt(i / 8) & mask) << (24 - i % 32);
  }
  return bin;
}
